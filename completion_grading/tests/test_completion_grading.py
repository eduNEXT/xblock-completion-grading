...
from completion_grading.utils import GradingMethod
...
class TestXBlockCompletionGrading(TestCase):
    ...
    def setUp(self) -> None:
        self.runtime = Mock(service=ToyRuntime().service)
    ...
    def test_calculate_grade(
        self, mock_create_submission, mock_emit_completion, mock_set_score
    ):
        """Test the JSON handler for calculating the grade.
    ...
    @unpack
    def test_get_raw_score(self, grading_method, unit_completions, expected_score):
        ...
        """
        self.block.get_unit_completions = Mock(return_value=5)
        self.block.unit_completions = unit_completions
        self.block.grading_method = grading_method

        raw_score = self.block.get_raw_score()

        self.assertEqual(raw_score, expected_score)

    @patch("completion_grading.completion_grading.get_course_sequences")
    def test_get_unit_completions(self, mock_get_course_sequences):
        ...
        completed_units = self.block.get_unit_completions()
